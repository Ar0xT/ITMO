...
public class BandRepository {
    ...
    public BandRepository(DatabaseClientProxy db, StudioRepository studioRepository) {
        this.db = db;
    ...
    public BandInsertResult insert(String key, MusicBand band, String ownerLogin) throws SQLException {
        return db.executeInTransaction(connection -> {
            ...
            try (PreparedStatement stmt = connection.prepareStatement(
                ...
                if (band.getSinglesCount() == null) {
                    ...
                    stmt.setInt(6, band.getSinglesCount());
                }
                if (band.getEstablishmentDate() == null) {
                    stmt.setNull(7, java.sql.Types.DATE);
                } else {
                    stmt.setDate(7, Date.valueOf(band.getEstablishmentDate()));
                }
    ...
    public boolean update(long id, MusicBand band, String ownerLogin) throws SQLException {
        Integer updated = db.executeInTransaction(connection -> {
            Long studioId = studioRepository.findOrCreate(connection, band.getStudio() == null ? null : band.getStudio().getName());
            try (PreparedStatement stmt = connection.prepareStatement(
                ...
                if (band.getEstablishmentDate() == null) {
                    ...
                    stmt.setDate(6, Date.valueOf(band.getEstablishmentDate()));
                }
                if (band.getGenre() == null) {
                    stmt.setNull(7, java.sql.Types.VARCHAR);
                } else {
                    stmt.setString(7, band.getGenre().name());
                }
                if (studioId == null) {
                    stmt.setNull(8, java.sql.Types.BIGINT);
                } else {
                    stmt.setLong(8, studioId);
                }
                stmt.setLong(9, id);
                stmt.setString(10, ownerLogin);
    ...
    public int deleteByKey(String key, String ownerLogin) throws SQLException {
        return db.execute(connection -> {
            try (PreparedStatement stmt = connection.prepareStatement(
    ...
    public int deleteByKeys(List<String> keys, String ownerLogin) throws SQLException {
        if (keys.isEmpty()) {
            return 0;
        }
        return db.executeInTransaction(connection -> {
    ...
    private MusicBand mapBand(ResultSet rs) throws SQLException {
        ...
        java.time.LocalDate establishment = null;
        Date establishmentDate = rs.getDate("establishment_date");
        if (establishmentDate != null) {
            establishment = establishmentDate.toLocalDate();
        }
        ...
        Timestamp created = rs.getTimestamp("creation_date");
        if (created != null) {
            band.setCreationDate(new java.util.Date(created.getTime()));
        }
